...
{
    return rt_pin_read(KEY_1_PIN);
}

static uint8_t button_key1_read(void)
{
    return rt_pin_read(KEY_2_PIN);
}
...
{
    return rt_pin_read(KEY_WAKEUP_PIN);
}
...
static void user_button_init(void)
{
    ...
    user_button[USER_BUTTON_2].usr_button_read = button_keywkup_read;

    rt_pin_mode(KEY_1_PIN, PIN_MODE_INPUT); /* set KEY pin mode to input */
    rt_pin_mode(KEY_2_PIN, PIN_MODE_INPUT); /* set KEY pin mode to input */
    rt_pin_mode(KEY_WAKEUP_PIN, PIN_MODE_INPUT); /* set KEY pin mode to input */
